...
make -j8 -k  CFLAGS_MODULE="-fno-pic" -C $SRC O=$KERNEL_OUT ARCH=arm CROSS_COMPILE="`which ccache` $GCC" zImage-dtb
...
cat $KERNEL_OUT/arch/arm/boot/zImage-dtb > $ANDROID_OUT/target/product/i9300/kernel
